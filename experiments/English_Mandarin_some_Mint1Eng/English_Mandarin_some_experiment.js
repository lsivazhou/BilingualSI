...
const Mandarin_Mint = {
    timeline: [
        {   type: jsPsychHtmlKeyboardResponse,
            choices: [' '],
            stimulus: `现在您将用中文做一遍这项任务。接下来您将看到8行图片。从<strong>左上角</strong>开始，请试着用中文说出每个东西的名称，并且请尽可能在不犯错的情况下快速回答。如果有任何您不知道或不记得的名称，请说“不知道”或“跳过”，然后继续说剩下图片的名称。如果在跳过一些图片之后想起了之前图片的正确名称，可以随时重新说那张图片正确名称（例如：之前的图是...）。请在<strong>2分钟</strong>内命名尽可能多的图片。`,
            response_ends_trial: true,
